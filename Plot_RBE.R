...
for(file in LET_cycle){
  ...
  if(file == LET_cycle[4] || file == LET_cycle[5] ||file == LET_cycle[11]){
...
plotRBE10<-cbind(plotRBE10,LET,yD,Type,alpha,beta,ratio)
#colnames(plotRBE10)[5]<-"MID"
colnames(plotRBE10)[5]<-"alpha"
colnames(plotRBE10)[6]<-"beta"
colnames(plotRBE10)[7]<-"ratio"
...
ggplot(plot,aes(LET,RBE10,color=Type,shape=Type))+
           geom_line(linewidth=0)+geom_point(size=2)+
           scale_y_log10()+scale_color_manual(values=c25)+
           xlab("LET [keV/um]")+ylab("RBE10")+annotation_logticks(sides = "l")+
           labs(title = "RBE10 (Carbon ions)",
                colour = "Data set")+
           guides(shape="none")+
