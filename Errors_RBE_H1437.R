...
for(file in LET_cycle){
  if(ion=="He"){
    S_real<-pide_data$H1437$He[[file]]$LQ
    dose<-pide_data$H1437$He[[file]]$Dose
    err<-pide_data$H1437$He[[file]]$ErrSF
    S_up<-S_real+0.5*err
    S_down<-S_real-0.5*err
  }else if(ion=="H"){
    S_real<-pide_data$H1437$H[[file]]$LQ
    dose<-pide_data$H1437$H[[file]]$Dose
    err<-pide_data$H1437$H[[file]]$ErrSF
    S_up<-S_real+0.5*err
    S_down<-S_real-0.5*err
  }else if(ion=="C"){
    S_real<-pide_data$H1437$C[[file]]$LQ
    dose<-pide_data$H1437$C[[file]]$Dose
    err<-pide_data$H1437$C[[file]]$ErrSF
    S_up<-S_real+0.5*err
    S_down<-S_real-0.5*err
  }
  ...
  # FIT for ERROR
  if(ion == "H"){
    quadratic_fit_exp <- LET_cycle[-c(1)]
    quadratic_fit_mod <- LET_cycle[-c(1,2,8,11,12)]
  }
  if(ion == "He"){
    quadratic_fit_exp <- LET_cycle[7]
    quadratic_fit_mod <- LET_cycle[-c(4,5,8,10,11)]
  }
  if(ion == "C"){
    quadratic_fit_exp <- LET_cycle[1]
    quadratic_fit_mod <- LET_cycle[c(1,2,6,7)]
  }
  ...
  is.na(beta_up)<-0
  alpha_down<-c(alpha_down,mod_dwon$coefficients[1])
  is.na(alpha_down)<-0
  ...
  if(ion == "H"){
    ...
    if(file == LET_cycle[1] || file == LET_cycle[2] || file == LET_cycle[8] || file == LET_cycle[11] || file == LET_cycle[12]){
  ...
  if(file == LET_cycle[4] || file == LET_cycle[5] || file == LET_cycle[6] || file == LET_cycle[7] || file == LET_cycle[8] || file == LET_cycle[10] || file == LET_cycle[11] ){
    d10_mod<-c(d10_mod,log(survperc)/mod1$coefficients[1])
    beta_mod[ctr]<-0
  }else{
    d10_mod<-c(d10_mod,quadraticRoots(1,mod1$coefficients[1]/mod1$coefficients[2],-log(survperc)/mod1$coefficients[2]))
  ...
  if(ion == "C"){
    ...
    if(file != LET_cycle[1] && file != LET_cycle[2] && file != LET_cycle[6] && file != LET_cycle[7]){
      d10_mod<-c(d10_mod,log(survperc)/mod1$coefficients[1])
      beta_mod[ctr]<-0
    }else{
      d10_mod<-c(d10_mod,quadraticRoots(1,mod1$coefficients[1]/mod1$coefficients[2],-log(survperc)/mod1$coefficients[2]))
...
# Unmeaningful data (1 as fixed assigned number)
D10_up[1,1]<-1
D10_down[1,1]<-1
...
#plot <- plot %>% filter(plot$LET != LET_cycle[5])
#plot <- plot %>% filter(plot$LET != LET_cycle[7])
#plot <- plot %>% filter(plot$LET != LET_cycle[8])
...
ggplot(plot,aes(yD,RBE10,color=Type,shape=Type))+
           ...
           labs(title = "RBE10 Protons (H1437)",
                colour = "Data set")+
           guides(shape="none")+
...
setwd("/home/user/Scrivania/Dottorato/RISULTATI_FIT/Global/")
save(plotRBE10, file = "FINAL_C_H1437.RData")
