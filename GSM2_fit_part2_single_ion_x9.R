...
for (rd in domain_size) {
  for (Rn in domain_size_cell) {
    ...
    result_<-optim(c(0.01,0.1,2),lower=c(0.00001,0.00001,1),upper=c(1,1,16), 
...
#save(surv_GSM2, file = "/home/user/Scrivania/Dottorato/RISULTATI_FIT/RBE/Surv_mod_C.RData")
...
for (file in coloumn_corrected) {
  ...
  #la dose diventa la dose contenuta nel file survival rispetto alla linea cellulare, allo ione e all'energia
  if(ion=="He"){
    dose_list_all<-c(dose_list_all,pide_data[[cell]]$He[[energy_list[ctr_e]]]$Dose)
    energy_list_all<-c(energy_list_all,rep(energy_list[ctr_e],length(pide_data[[cell]]$He[[energy_list[ctr_e]]]$Dose)))
  }else if(ion=="H"){
    dose_list_all<-c(dose_list_all,pide_data[[cell]]$H[[energy_list[ctr_e]]]$Dose)
    energy_list_all<-c(energy_list_all,rep(energy_list[ctr_e],length(pide_data[[cell]]$H[[energy_list[ctr_e]]]$Dose)))
  }else if(ion=="C"){
    dose_list_all<-c(dose_list_all,pide_data[[cell]]$C[[energy_list[ctr_e]]]$Dose)
    energy_list_all<-c(energy_list_all,rep(energy_list[ctr_e],length(pide_data[[cell]]$C[[energy_list[ctr_e]]]$Dose)))
  }
  ctr_e<-ctr_e+1
}
...
write.csv(Fit_parameters, "/home/user/Scrivania/Dottorato/RISULTATI_FIT/Good/Carbon/Carbon_fit_1_3_6_9.csv", row.names=FALSE)
...
for (file in as.numeric(coloumn_corrected)) {
  error_up<-c(error_up,survival[[cell]][[ion]][[file]]$LQ + 0.5*survival[[cell]][[ion]][[file]]$ErrSF)
  error_down<-c(error_down,survival[[cell]][[ion]][[file]]$LQ - 0.5*survival[[cell]][[ion]][[file]]$ErrSF)
}
